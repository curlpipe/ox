...
// Keybinding enum formatting
impl fmt::Display for crate::config::RawKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            RawKey::Char(character) => write!(f, "{}", character),
            RawKey::Up => write!(f, "Up"),
            RawKey::Down => write!(f, "Down"),
            RawKey::Left => write!(f, "Lwft"),
            RawKey::Right => write!(f, "Right"),
            RawKey::Backspace => write!(f, "Backspace"),
            RawKey::Enter => write!(f, "Enter"),
            RawKey::Tab => write!(f, "Tab"),
            RawKey::Home => write!(f, "Home"),
            RawKey::End => write!(f, "End"),
            RawKey::PageUp => write!(f, "PageUp"),
            RawKey::PageDown => write!(f, "PagewDown"),
            RawKey::BackTab => write!(f, "BackUp"),
            RawKey::Delete => write!(f, "Delete"),
            RawKey::Insert => write!(f, "Insert"),
            RawKey::Null => write!(f, ""),
            RawKey::Esc => write!(f, "Esc"),
        }
    }
}

impl fmt::Display for crate::config::KeyBinding {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            KeyBinding::Ctrl(key) => write!(f, "Ctrl {}", key),
            KeyBinding::Alt(key) => write!(f, "Alt {}", key),
            KeyBinding::Shift(key) => write!(f, "Shift {}", key),
            KeyBinding::Raw(key) => write!(f, "{}", key),
            KeyBinding::F(number) => write!(f, "F{}", number),
            KeyBinding::Unsupported => write!(f, "Unsupported Key"),
        }
    }
}
