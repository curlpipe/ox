...
pub struct Editor {
    ...
    docs: Vec<Document>,            // For holding our document
    current_doc: usize,             // Index of the current document
...
impl Editor {
    pub fn new(args: App) -> Result<Self, Error> {
        ...
        Ok(Self {
            ...
            docs: if files.is_empty() {
                vec![Document::new(&config.0)]
            } else {
                files
                    .iter()
                    .map(|file| Document::from(&config.0, file))
                    .collect()
            },
            current_doc: 0,
            last_keypress: None,
            ...
            if let Some(key) = self.stdin.next() {
                ...
                if let Some(time) = self.last_keypress {
                    if time.elapsed().as_secs() >= self.config.general.undo_period {
                        self.docs[self.current_doc].undo_stack.commit();
    ...
    fn redo(&mut self) {
        if let Some(events) = self.docs[self.current_doc].redo_stack.pop() {
            for event in events.iter().rev() {
                match event {
                    ...
                    Event::InsertMid(pos, c) => {
                        ...
                        self.recalculate_graphemes();
                        self.docs[self.current_doc].rows[pos.y].insert(*c, pos.x);
                    }
                    Event::BackspaceMid(pos, _) => {
                        self.cursor.y = pos.y - self.offset.y;
                        self.cursor.x = pos.x - self.offset.x;
                        self.recalculate_graphemes();
                        self.docs[self.current_doc].rows[pos.y].delete(pos.x);
                    }
                    Event::ReturnEnd(pos) => {
                        self.cursor.y = pos.y - self.offset.y;
                        self.cursor.x = pos.x - self.offset.x;
                    ...
                    Event::ReturnMid(pos, breakpoint) => {
                        ...
                        self.cursor.x = pos.x - self.offset.x;
                        self.recalculate_graphemes();
                        let current = self.docs[self.current_doc].rows[pos.y].string.clone();
                        let before = Row::from(&current[..*breakpoint]);
                        let after = Row::from(&current[*breakpoint..]);
                        self.docs[self.current_doc].rows.insert(pos.y + 1, after);
                        self.docs[self.current_doc].rows[pos.y] = before;
                        self.move_cursor(Key::Down);
                    ...
                    Event::BackspaceStart(pos) => {
                        self.cursor.y = pos.y - self.offset.y;
                        self.recalculate_graphemes();
                        let current = self.docs[self.current_doc].rows[pos.y + 1].string.clone();
                        let prev = self.docs[self.current_doc].rows[pos.y].clone();
                        self.docs[self.current_doc].rows[pos.y + 1] =
                            Row::from(&(prev.string.clone() + &current)[..]);
                        self.docs[self.current_doc].rows.remove(pos.y);
                        self.move_cursor(Key::Up);
                        self.cursor.x = prev.length();
                        self.recalculate_graphemes();
                    }
                    Event::UpdateLine(pos, _, after) => {
                        self.docs[self.current_doc].rows[*pos] = after.clone();
                        self.snap_cursor();
                        self.prevent_unicode_hell();
                        self.recalculate_graphemes();
                    }
                }
                self.docs[self.current_doc].dirty = true;
                self.show_welcome = false;
            }
            self.docs[self.current_doc].undo_stack.append(events);
        } else {
    ...
    fn undo(&mut self) {
        self.docs[self.current_doc].undo_stack.commit();
        if let Some(events) = self.docs[self.current_doc].undo_stack.pop() {
            for event in &events {
                match event {
                    // TODO: Update relavent lines here
                    Event::InsertTab(pos) => {
                        for i in 1..=self.config.general.tab_width {
                            self.docs[self.current_doc].rows[pos.y].delete(pos.x - i);
                    ...
                    Event::InsertMid(pos, c) => {
                        ...
                        self.cursor.x = pos.x.saturating_add(c_len) - self.offset.x;
                        self.recalculate_graphemes();
                        let string = self.docs[self.current_doc].rows[pos.y].string.clone();
                        self.docs[self.current_doc].rows[pos.y]
                    ...
                    Event::ReturnEnd(pos) => {
                        self.docs[self.current_doc].rows.remove(pos.y + 1);
                        self.move_cursor(Key::Up);
                    ...
                    Event::ReturnMid(pos, breakpoint) => {
                        let current = self.docs[self.current_doc].rows[pos.y].string.clone();
                        let after = self.docs[self.current_doc].rows[pos.y + 1].string.clone();
                        self.docs[self.current_doc].rows.remove(pos.y);
                        self.docs[self.current_doc].rows[pos.y] =
                            Row::from(&(current + &after)[..]);
                        self.move_cursor(Key::Up);
                        self.leap_cursor(Key::Home);
                    ...
                    Event::BackspaceStart(pos) => {
                        let before =
                            Row::from(&self.docs[self.current_doc].rows[pos.y].string[..pos.x]);
                        let after =
                            Row::from(&self.docs[self.current_doc].rows[pos.y].string[pos.x..]);
                        self.docs[self.current_doc].rows[pos.y] = after;
                        self.docs[self.current_doc].rows.insert(pos.y, before);
                        self.move_cursor(Key::Down);
                        self.leap_cursor(Key::Home);
                    }
                    Event::UpdateLine(pos, before, _) => {
                        self.docs[self.current_doc].rows[*pos] = before.clone();
                        self.snap_cursor();
                        self.prevent_unicode_hell();
                        self.recalculate_graphemes();
                    }
            ...
            self.docs[self.current_doc].redo_stack.append(events);
        } else {
    ...
    fn character(&mut self, c: char) {
        ...
        match c {
            ...
            _ => {
                ...
                self.docs[self.current_doc].dirty = true;
                self.show_welcome = false;
                self.docs[self.current_doc].rows[self.cursor.y + self.offset.y]
                    .insert(c, self.graphemes);
                self.docs[self.current_doc]
                    .undo_stack
                    .push(Event::InsertMid(
                        Position {
                            x: self.cursor.x + self.offset.x,
                            y: self.cursor.y + self.offset.y,
                        },
        ...
        self.docs[self.current_doc].redo_stack.empty();
    }
    fn tab(&mut self) {
        // Insert a tab
        // TODO: Update relavent lines here
    ...
    fn return_key(&mut self) {
        ...
        if self.cursor.x + self.offset.x == 0 {
            ...
            == self.docs[self.current_doc].rows[self.cursor.y + self.offset.y].length()
        {
            ...
            let current = self.docs[self.current_doc].rows[self.cursor.y + self.offset.y].chars();
            ...
            self.docs[self.current_doc].rows[self.cursor.y + self.offset.y] = before.clone();
            self.docs[self.current_doc]
                .undo_stack
                .push(Event::ReturnMid(
                    Position {
                        x: self.cursor.x + self.offset.x,
                        y: self.cursor.y + self.offset.y,
                    },
    ...
    fn backspace(&mut self) {
        ...
        if self.cursor.x + self.offset.x == 0 && self.cursor.y + self.offset.y != 0 {
            ...
            let current = self.docs[self.current_doc].rows[self.cursor.y + self.offset.y]
                .string
                .clone();
            let prev = self.docs[self.current_doc].rows[self.cursor.y + self.offset.y - 1].clone();
            self.docs[self.current_doc].rows[self.cursor.y + self.offset.y - 1] =
                Row::from(&(prev.string.clone() + &current)[..]);
            self.docs[self.current_doc]
                .rows
            ...
            self.move_cursor(Key::Left);
            let ch = self.docs[self.current_doc].rows[self.cursor.y + self.offset.y].clone();
            self.docs[self.current_doc].rows[self.cursor.y + self.offset.y].delete(self.graphemes);
    ...
    fn quit(&mut self) {
        let index = self.docs.iter().enumerate().find(|(_, doc)| doc.dirty);
    ...
    fn new_document(&mut self) {
        ...
        if self.dirty_prompt('n', "new") {
            ...
            self.docs.push(Document::new(&self.config));
            self.current_doc = index;
            self.docs[self.current_doc].dirty = false;
            self.cursor.y = 0;
            self.offset.y = 0;
            self.leap_cursor(Key::Home);
        }
    ...
    fn close(&mut self) {
        // For handling a quit event
        if self.dirty_prompt('c', "close") {
            if self.docs.len() == 1 {
                self.quit = true;
            } else {
                self.docs.remove(self.current_doc);
                self.current_doc = self.current_doc.saturating_sub(1);
            }
        }
    }
    fn save(&mut self) {
        // Handle save event
        if self.docs[self.current_doc].save().is_ok() {
            self.docs[self.current_doc].dirty = false;
            self.set_command_line(
                format!(
                    "File saved to {} successfully",
                    self.docs[self.current_doc].path
                ),
                Type::Info,
            );
        } else {
    ...
    fn save_as(&mut self) {
        ...
        if let Some(result) = self.prompt("Save as", &|_, _, _| {}) {
            if self.docs[self.current_doc].save_as(&result[..]).is_ok() {
                self.docs[self.current_doc].dirty = false;
                self.set_command_line(format!("File saved to {} successfully", result), Type::Info);
                self.docs[self.current_doc].name = result.clone();
                self.docs[self.current_doc].path = result;
            } else {
    ...
    fn search(&mut self) {
        ...
        self.prompt("Search", &|s, e, t| {
            let search_points = s.docs[s.current_doc].scan(t);
            match e {
    ...
    fn replace(&mut self) {
        ...
        if let Some(target) = self.prompt("Replace", &|_, _, _| {}) {
            if let Some(arrow) = self.prompt("With", &|_, _, _| {}) {
                ...
                let mut search_points = self.docs[self.current_doc].scan(&target);
                for p in &search_points {
                ...
                loop {
                    ...
                    match key {
                        ...
                        Key::Char('\n') | Key::Char('y') | Key::Char(' ') => {
                            self.docs[self.current_doc].undo_stack.commit();
                            let line = self.docs[self.current_doc].rows
                                [self.cursor.y + self.offset.y]
                                .clone();
                            let before = self.docs[self.current_doc].rows
                                [self.cursor.y + self.offset.y]
                                .clone();
                            ...
                            self.prevent_unicode_hell();
                            self.recalculate_graphemes();
                            search_points = self.docs[self.current_doc].scan(&target);
                        }
                        Key::Esc => break,
                        _ => (),
                    }
    ...
    fn replace_all(&mut self) {
        if let Some(target) = self.prompt("Replace", &|_, _, _| {}) {
            if let Some(arrow) = self.prompt("With", &|_, _, _| {}) {
                self.docs[self.current_doc].undo_stack.commit();
                let re = Regex::new(&target).unwrap();
                let lines = self.docs[self.current_doc].rows.clone();
                for (c, line) in lines.iter().enumerate() {
                    let before = self.docs[self.current_doc].rows[c].clone();
                    let after = Row::from(&*re.replace_all(&line.string[..], &arrow[..]));
                    if before.string != after.string {
                        self.docs[self.current_doc]
                            .undo_stack
                            .push(Event::UpdateLine(c, before.clone(), after.clone()));
                        // TODO: Update relavent lines here
                        self.docs[self.current_doc].rows[c] = after;
                    }
                }
            }
        }
    ...
    fn next_doc(&mut self) {
        ...
        if self.current_doc == self.docs.len() {
            self.current_doc = 0;
        }
    }
    ...
    fn dirty_prompt(&mut self, key: char, subject: &str) -> bool {
        ...
        if self.docs[self.current_doc].dirty {
        ...
        match action {
            ...
            Key::PageDown => {
                self.cursor.y = cmp::min(
                    self.docs[self.current_doc].rows.len().saturating_sub(1),
            ...
            Key::End => {
                let line = &self.docs[self.current_doc].rows[self.cursor.y + self.offset.y];
                if line.length()
                    >= self
                        .term
                        .width
                        .saturating_sub(self.docs[self.current_doc].line_offset as u16)
                        as usize
                {
                    ...
                    self.offset.x = line
                        .length()
                        .saturating_add(jump + self.docs[self.current_doc].line_offset + 1)
                        .saturating_sub(self.term.width as usize);
                    self.cursor.x =
                        self.term.width.saturating_sub(
                            (jump + self.docs[self.current_doc].line_offset + 1) as u16,
                        ) as usize;
                } else {
    ...
    fn move_cursor(&mut self, direction: Key) {
        ...
        match direction {
            Key::Down => {
                if self.cursor.y + self.offset.y + 1 < self.docs[self.current_doc].rows.len() {
            ...
            Key::Right => {
                ...
                if line.length() > self.cursor.x + self.offset.x {
                    ...
                    let indicator1 = self.cursor.x
                        == self.term.width.saturating_sub(
                            (self.docs[self.current_doc].line_offset + jump + 1) as u16,
                        ) as usize;
                    // Check for half broken unicode character
                    let indicator2 =
                        self.cursor.x
                            == self.term.width.saturating_sub(
                                (self.docs[self.current_doc].line_offset + jump) as u16,
                            ) as usize;
    ...
    fn snap_cursor(&mut self) {
        ...
        let current = self.docs[self.current_doc].rows[self.cursor.y + self.offset.y].clone();
    ...
    fn goto(&mut self, pos: &Position) {
        ...
        let max_x =
            (self.term.width as usize).saturating_sub(self.docs[self.current_doc].line_offset);
    ...
    fn update(&mut self) {
        ...
        self.docs[self.current_doc].recalculate_offset(&self.config);
        self.render();
        self.term.goto(&Position {
            x: self
                .cursor
                .x
                .saturating_add(self.docs[self.current_doc].line_offset),
            y: self.cursor.y,
        });
        self.term.flush();
    }
    ...
    fn status_line(&mut self) -> String {
        ...
        let left = format!(
            ...
            self.docs[self.current_doc].identify(),
            self.current_doc + 1,
            self.docs.len()
        );
        ...
        let right = format!(
            ...
            self.docs[self.current_doc].rows.len(),
    ...
    fn render(&mut self) {
        ...
        for row in 0..self.term.height {
            if let Some(row) = self.docs[self.current_doc]
                .rows
            ...
            } else if let Some(line) = self.docs[self.current_doc]
                .rows
                .get(self.offset.y + row as usize)
            {
                ...
                frame.push(self.add_background(&line.render(
                    ...
                    self.docs[self.current_doc].line_offset,
                    &self.config,
                )));
            } else {
                // Render empty lines
